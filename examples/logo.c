...
printf("%lu\n", sizeof(buffer));
  for(int32_t y = 0; y < tile.bounds.h; y++) {
    for(int32_t x = 0; x < tile.bounds.w; x++) {     
      ...
      buffer[y + tile.bounds.y][x + tile.bounds.x] = blend(buffer[y + tile.bounds.y][x + tile.bounds.x], alpha_pen);
...
int main() {
  ...
  for(int i = 220; i <= 360; i += 10) {
    ...
    if(i == 360) {
      pp_mat3_t shadow_transform = transform;      
      shadow_transform = pp_mat3_mul(shadow_transform, pp_mat3_translation(75, 75));
      pp_transform(&shadow_transform);
