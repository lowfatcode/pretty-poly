...
// count
pp_point_t last;
pp_path_t parse_svg_path_contour(char **caret) {  
  
  pp_path_t result;
  result.count = get_svg_path_point_count(*caret);
  result.points = malloc(sizeof(pp_point_t) * result.count);
...
pp_poly_t parse_svg_path(char *path) {
  pp_poly_t result;
  result.count = get_svg_path_count(path);
  result.paths = malloc(sizeof(pp_path_t) * result.count);
